...
train = train.iloc[:-1]
train.to_parquet(f"{processed_path}/train.parquet", index=True)

validation = data[(data['timestamp'] >= train_date) & (data['timestamp'] < validation_date)]
validation = validation.iloc[:-1]
validation.to_parquet(f"{processed_path}/validation.parquet", index=True)

test = data[(data['timestamp'] >= validation_date)]
test.to_parquet(f"{processed_path}/test.parquet", index=True)